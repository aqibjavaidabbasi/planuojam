...
function ClientVenueWrapper({ venueNames }: { venueNames: string[] }) {
  ...
  const translatedPricingFilters = [
  ...
  const filters = [
    {
      name: "category",
      options: venueNames,
      placeholder: t("filters.chooseVenue"),
    },
    {
      name: "price",
      options: translatedPricingFilters,
      placeholder: t("filters.selectPricing"),
    },
    {
      name: "eventType",
      options: eventTypeNames,
      placeholder: t("filters.chooseEventType"),
    },
  ];
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-5 my-10 flex-wrap">
        {venueList.length === 0 ? (
          <NoDataCard>{t("noVenuesFound")}</NoDataCard>
        ) : (
          venueList.map((venue) => (
