...
async function fetchWithLocaleFallback(
  ...
  if (locale) {
    const queryWithLocale = createQuery(populate, {
      locale,
      ...additionalParams,
    });
    const resLocale = await fetchAPI(resource, queryWithLocale, filters);
    if (Array.isArray(resLocale) ? resLocale.length : !!resLocale)
      return resLocale;
  }

  // 2) Fallback to DEFAULT_LOCALE (if different from requested)
  if (!locale || locale !== DEFAULT_LOCALE) {
    const queryDefault = createQuery(populate, {
      locale: DEFAULT_LOCALE,
      ...additionalParams,
    });
    const resDefault = await fetchAPI(resource, queryDefault, filters);
    if (Array.isArray(resDefault) ? resDefault.length : !!resDefault)
      return resDefault;
  }
...
export async function fetchAllChildCategories(
  locale?: string,
  pageSize: number = 200,
) {
  const filter = {
        ...
        .map((item) => {
          ...
          if (Array.isArray(locs) && displayLocale) {
            const match = locs.find(
              (l) => l?.attributes?.locale === displayLocale,
            );
