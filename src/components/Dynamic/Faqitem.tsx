...
function Faqitem({ faq, idx, isOpen, setOpenIndexes }: FaqitemProps) {
  ...
  return (
    <div
      className="rounded-lg border border-gray-200 shadow-sm transition h-fit w-full mt-2"
    >
      <button
      ...
      >
        ...
        <FaChevronDown
          className={`w-5 h-5 text-primary transform transition-transform duration-200 ${isOpen ? 'rotate-180' : ''
            }`}
          aria-hidden="true"
        />
      </button>
