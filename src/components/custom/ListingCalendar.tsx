...
const ListingCalendar: React.FC<ListingCalendarProps> = ({
  listingDocumentId,
  workingSchedule,
}) => {
  ...
  const fetchRange = useCallback(
    async (rangeStartISO: string, rangeEndISO: string) => {
      ...
      try {
        setLoading(true);
        const bookings = await getListingBookingsPublic(
          listingDocumentId,
          rangeStartISO,
          rangeEndISO,
        );
        const bookedBackgrounds = (bookings || []).flatMap(
          expandBookingToDayBackgrounds,
        );
        ...
        const windowsForDay = (d: Date) =>
          schedule.filter((w) => w.day === dayKey(d));
        ...
        while (cur.getTime() < end.getTime()) {
          ...
          if (
            currentViewType.current === 'dayGridMonth' ||
            currentViewType.current === 'multiMonthYear'
          ) {
            if (!inBooked) {
  ...
  return (
    <div className='bg-white rounded-xl shadow-sm p-3 md:p-4 lg:p-6'>
      <h2 className='text-2xl font-semibold text-primary mb-4'>
        {t('availability', { default: 'Availability' })}
      </h2>
      <div className='flex items-center gap-3 mb-3 text-sm'>
        <div className='flex items-center gap-2'>
          <span
            className='inline-block w-4 h-4 rounded-sm'
            style={{ backgroundColor: 'rgba(0, 123, 255, 0.7)' }}
          />
          <span className='text-gray-600'>
            {t('legend.booked', { default: 'Booked (shaded days)' })}
          </span>
        </div>
        <div className='flex items-center gap-2'>
          <span
          ...
          <span className='text-gray-600'>
            {t('legend.unavailable', {
              default: 'Unavailable (no working schedule)',
            })}
          </span>
        </div>
        <div className='flex items-center gap-2'>
          ...
          <span className='text-gray-600'>
            {t('legend.available', {
              default: 'Available (has working schedule)',
            })}
          </span>
        </div>
      </div>
      ...
      <div className='mt-2 h-5'>
        {loading && (
          <div className='text-sm text-gray-500'>
            {t('loading', { default: 'Loading calendar...' })}
          </div>
        )}
      </div>
    </div>
