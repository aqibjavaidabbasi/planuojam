...
const FiltersAndMap = dynamic(
  () => import('@/components/global/FiltersAndMap'),
  { ssr: false },
);
const ListingCard = dynamic(() => import('@/components/Dynamic/ListingCard'));
const NoDataCard = dynamic(() => import('@/components/custom/NoDataCard'), {
  ssr: false,
});
...
function ClientListingWrapper({
  ...
  useEffect(() => {
    ...
    if (initialPagination?.total !== undefined)
  ...
  const placeholders = useMemo(
    () => ({
      ...
      chooseEventType:
        serviceType === 'vendor'
          ? 'filters.pickEventType'
          : serviceType === 'venue'
            ? 'filters.chooseEventType'
            : 'filters.pickEventType',
      emptyList:
        serviceType === 'vendor'
          ? 'noVendorsFound'
        ...
        <div className='flex items-center justify-center my-6'>
          <LoadMoreButton
            onClick={async () => {
              ...
              try {
                const nextPage =
                  (Number.isFinite(page as number) ? page : 1) + 1;
                ...
                const newItems = Array.isArray(resp?.data)
                  ? (resp.data as ListingItem[])
                  : [];
                const meta = resp?.meta?.pagination;
