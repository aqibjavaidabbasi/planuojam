...
export default function BasicSection({
  ...
  const serviceType = (serviceTypeRaw || 'vendor').toLowerCase().trim() as
    | 'vendor'
    | 'venue';
  ...
  useEffect(() => {
    const populateTags = async () => {
      if (listing?.tagDocumentIds?.length) {
        try {
          const tags = await fetchTagsByIds(
            listing.tagDocumentIds as string[],
            locale,
          );
    ...
    const fetchSubscriptionPriceRange = async () => {
      try {
        ...
        const subResponse = await fetch(
          `/api/subscription-status?listingDocId=${listing.documentId}`,
        );
        ...
        if (
          ...
          const products: StripeProductAttributes[] = Array.isArray(
            productsData,
          )
            ? productsData
            : productsData.data || [];
    ...
    try {
      ...
      if (priceRange && values.price != null && !isNaN(values.price)) {
        if (values.price < priceRange.min) {
          setSubmitting(false);
          return toast.error(
            t('errors.priceBelowMin', { min: priceRange.min }),
          );
        }
        if (values.price > priceRange.max) {
          setSubmitting(false);
          return toast.error(
            t('errors.priceAboveMax', { max: priceRange.max }),
          );
        }
      }
        ...
        for (const day of Object.keys(byDay)) {
          ...
          for (let i = 1; i < ranges.length; i++) {
            ...
            if (cur.s < prev.e) {
              setSubmitting(false);
              return toast.error(
                tModal('workingSchedule.errors.conflictDay', {
                  day: tModal(`days.${day}`),
                }),
              );
            }
                ...
                <Input
                  ...
                  {...register(`videos.${idx}.url`, {
                    pattern: {
                      ...
                      message:
                        t('errors.invalidYouTubeUrl') || 'Invalid YouTube URL',
                    },
                  })}
                />
                <Button
            ...
            >
              <div className='md:col-span-3'>
                <Select
                  ...
                  options={[
                    {
                      label: tModal('workingSchedule.placeholder') as string,
                      value: '',
                    },
